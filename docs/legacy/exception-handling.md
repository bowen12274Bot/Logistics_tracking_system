...
> 注意：此文件為舊版規範（Legacy）。現行異常規則已整併到 `docs/modules/exceptions.md`，並以該文件為準。
...
### 4.2 完整 reason_code（本專案 MVP）
本專案的模擬配送事件：由寄件者建立出貨單開始，到收件者收貨/確認結束；因此異常分類先貼近此流程與需求。

| reason_code | 中文顯示（客戶可見） | 司機可申報 | 倉儲可申報 | 說明（何時用） |
|---|---|---:|---:|---|
...
| `other` | 其他（請詳述） | ✅ | ✅ | 不在以上分類，必填 description |

> 規範決定：客戶端只顯示「中文分類」（reason_code → 中文），不顯示 description / handling_report。

### 4.3 角色可用 reason_code（清單）
- 司機可申報：`lost`、`damaged`、`unpaid`、`sender_not_ready`、`no_answer`、`refused`、`address_issue`、`other`
- 倉儲可申報：`lost`、`damaged`、`label_issue`、`misroute`、`other`

### 4.4 保留擴充（暫不啟用）

下列屬於「非包裹本身」或「營運因素」，目前不納入 UI 選項，但後續可加入而不影響資料結構：
- `vehicle_issue`（車輛故障）
- `delay`（延誤：交通/天候/塞車；本版先不啟用）
...
> 與「收件者未確認 7 天自動完成」的規則分離：該規則屬於 delivered 流程的完成判定（見 `docs/handbook/customer-service.md`），不影響 delivery_failed 的終端性。
...
- 異常成立後包裹需「停止流轉」並「留在當下」：
  - 若在收件端貨到付款拒收（`unpaid/refused`）等：留置在住家節點（location 為 `END_*`）。
...
客服的「異常池 → 決策 → 結案」流程與任務清單，請見 `docs/handbook/customer-service.md`。
