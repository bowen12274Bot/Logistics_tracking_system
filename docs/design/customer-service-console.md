...
- Primary 切換（兩顆切換鈕或 tabs）：
  - 異常（Exception）
  - 合約（Contract Review）
- Secondary 切換（沿用現有 `current/history` 概念）：
  - 現在（待處理）
  - 歷史（已結案/已審核）
...
### 5.1 推薦版型：兩欄工作台（列表 + 詳情），動作用 Modal

原因：既有 `UiModal` 可重用，且 `docs/design/ui-spec.md` 已明確建議「結案 modal」。
...
- Body（主區）
  - 左欄：任務列表（列表列設計見 6）
  - 右欄：任務詳情（純顯示資訊 + 主要行動按鈕）
  - 行動按鈕觸發 `UiModal`：
    - 異常：開「結案異常」modal（填寫與送出）
    - 合約：開「審核合約」modal

### 5.2 RWD（演示最低要求）

- 桌機：兩欄（列表/詳情）
- 小尺寸：右欄改成抽屜或改為「點列表進詳情頁面式展開」（保留同一 route）
