...
| M4   | 前端畫面設計      | 設計更美觀的前端頁面，套用新設計稿                 |        | 進行中   |
...
| P3   | 系統架構圖 | 畫出前端 / Workers / D1 / GitHub 架構         |        | 未開始   |
...
| T3  | 建立貨態事件 API                | 新增事件並更新當前狀態（共用內部服務，給司機/倉儲/客服/模擬使用）                                |        | 未開始   |
| T4  | 貨態查詢 API（含角色權限）      | 所有人皆可查貨態：客戶只能查自己的包裹；員工只能查與自己工作相關的包裹（司機路線、倉儲節點等）   |        | 未開始   |
...
| #  | 任務項目              | 說明                                                         | 負責人 | 實作狀態 |
| -- | --------------------- | ------------------------------------------------------------ | ------ | -------- |
| V1 | 虛擬地圖節點設計      | 定義配送中心、配送站、超商、住家等節點與屬性                |        | 未開始   |
| V2 | 地圖資料模型與 API    | 建立節點、連線、距離/時間成本資料並提供讀寫 API            |        | 未開始   |
