...
const tabContext = computed(() => {
  if (activeTab.value === "await_receive") {
    return {
      title: "點收清單",
      hint: "此區顯示本站已到站（warehouse_in）但尚未點收的包裹。",
    };
  }
  if (activeTab.value === "sorting") {
    return {
      title: "分揀工作區",
      hint: "點收後會進入此區，可決定下一跳並派發下一段任務。",
    };
  }
  return {
    title: "已派發（route_decided）",
    hint: "已建立下一段任務，等待司機接手。",
  };
});
...
const assignHintText = computed(() => {
  ...
  if (count === 0) return "拖曳左側 ⋮⋮ 到右側大框可分配（勾選多筆會一起拖曳）";
  return `已選取 ${count} 件；拖曳其中一筆的 ⋮⋮ 到右側大框即可分配`;
});
...
function assignSelectedToActiveTab() {
  ...
  if (!tab) {
    toast.warning("請先選擇右側配發地分頁。");
    return;
  }
  const ids = selectedUnassignedIds.value;
  if (ids.length === 0) {
    toast.warning("請先勾選左側要分配的包裹。");
    return;
  }
  for (const id of ids) {
...
function unassignAllForActiveTab() {
  const tab = String(activeDispatchTabId.value ?? "").trim();
  if (!tab) {
    toast.warning("請先選擇配發地分頁。");
    return;
  }
  const list = activeAssignedPackages.value;
  if (list.length === 0) {
    toast.info("此分頁目前沒有包裹可移回。");
    return;
  }
  for (const p of list) {
  ...
  toast.success(`已移回 ${list.length} 件包裹。`);
}
...
function autoAssignUnassigned() {
  ...
  if (selected.length === 0) {
    toast.info("請先勾選要自動分配的包裹。");
    return;
  }
  const list = selectedUnassignedPackages.value;
  if (list.length === 0) {
    toast.info("目前沒有可自動分配的勾選包裹。");
    return;
  }
  const counts = new Map<string, number>();
  ...
  toast.success(`已自動分配 ${list.length} 件包裹。`);
}
...
async function dispatchAllForActiveTab() {
  ...
  try {
    ...
    if (failed === 0) toast.success(`已派發 ${ok} 件`);
    else toast.warning(`派發完成（成功 ${ok}，失敗 ${failed}）`);
    await refresh();
  } finally {
    busy.value = false;
  }
  ...
  <UiPageShell eyebrow="員工 · 倉儲" title="站點與中心作業" lede="點收本站包裹、分揀並決定下一跳，派發司機轉運任務。">
    <div>
      <UiCard class="station-bar" style="margin-top: 16px">
            ...
            <div class="wh-summary">
              ...
              <div class="wh-summary-row">
                <div class="wh-station-chip" aria-label="station">
                  <span class="wh-station-chip__label">站點</span>
                  <strong class="wh-station-chip__id">{{ warehouseNodeId ?? "-" }}</strong>
                </div>
                <span v-if="busy" class="wh-summary-pill">處理中…</span>
              </div>
            </div>
          </div>
        ...
        <p v-else-if="loading" class="hint wh-gap-top-sm">載入中…</p>
      </UiCard>
      ...
      <div v-show="activeTab === 'await_receive'" class="wh-panel" role="tabpanel">
        <UiCard class="wh-receive-workspace">
          <div class="wh-receive-toolbar">
            <p class="hint wh-receive-selected">已選取 {{ selectedReceiveIds.length }} 件</p>
          ...
          <div v-if="loading" class="hint wh-gap-top-sm">載入中…</div>
          <div v-else-if="awaitingReceive.length === 0" class="wh-empty wh-gap-top-sm" style="margin-top: 10px">
            <p class="wh-empty-title">目前沒有待點收包裹</p>
            <p class="hint wh-empty-hint">本站沒有新的到站包裹，或已被點收。</p>
            <ul class="wh-empty-list">
              <li class="hint">點「重新整理」再確認</li>
              <li class="hint">切到「分揀中」查看待派發清單</li>
            </ul>
          </div>
      ...
      <div v-show="activeTab === 'sorting'" class="wh-panel" role="tabpanel">
        <UiCard class="wh-sort-workspace">
          <div class="wh-workspace-toolbar">
            <p class="hint" style="margin: 0">待分揀 {{ sorting.length }} 件</p>
          </div>
          ...
          <div class="wh-workspace-body">
            ...
            <div v-else-if="sorting.length === 0" class="wh-empty wh-gap-top-sm" style="margin-top: 10px">
              <p class="wh-empty-title">目前沒有待分揀包裹</p>
              <p class="hint wh-empty-hint">點收後包裹才會進入分揀工作區。</p>
              <ul class="wh-empty-list">
                <li class="hint">切到「待點收」查看到站包裹</li>
                <li class="hint">點「重新整理」更新狀態</li>
              </ul>
            </div>
            ...
            <UiCard class="wh-sort-list">
              <div class="wh-sort-list-fixed">
                <div class="wh-sort-filters">
                  <div class="wh-filter-row">
                    <p class="eyebrow" style="margin: 0">未分揀</p>
                    <span class="hint">{{ unassignedSortingPackages.length }} 件</span>
                  </div>

                  <div class="wh-filter-actions">
                    <button class="ghost-btn small-btn" type="button" :disabled="busy" @click="toggleUnassignedAll">
                      {{ allUnassignedChecked ? "取消全選" : "全選" }}
                    </button>
                    <button
                      class="ghost-btn small-btn"
                      type="button"
                    ...
                    >
                      自動整理分配（勾選 {{ selectedUnassignedIds.length }}）
                    </button>
              ...
              <div class="wh-sort-list-scroll">
                <div v-if="unassignedSortingPackages.length === 0" class="wh-empty">
                  <p class="wh-empty-title">目前沒有未分揀包裹</p>
                  <p class="hint wh-empty-hint">可點「自動整理分配」快速把待分揀包裹分配到右側各區。</p>
                </div>
                  ...
                  <li v-for="p in unassignedSortingPackages" :key="p.id" class="wh-unassigned-row">
                    <label class="wh-unassigned-left">
                      ...
                      <span class="wh-unassigned-main">
                        ...
                        <span class="hint wh-unassigned-meta">建議：{{ p.suggested_to_node_id ?? "-" }}</span>
                      </span>
                    </label>
              ...
              <div class="wh-sort-detail-fixed">
                <div class="wh-detail-head">
                  <div class="wh-detail-head__main">
                    <p class="wh-detail-title">配發地分類</p>
                    <p class="hint wh-detail-sub">將包裹分配到右側分頁，並支援批次/單筆派發。</p>
                  </div>
                  <div class="wh-detail-actions">
                    ...
                    <button
                      ...
                      :title="`全部派發（${activeAssignedPackages.length}）`"
                    >
                      全部派發（{{ activeAssignedPackages.length }}）
                    </button>
                  </div>
                </div>
                ...
                <div v-if="selectedUnassignedIds.length > 0" class="wh-selected-preview">
                  <template v-if="singleSelectedUnassignedPackage">
                    ...
                    <p class="hint" style="margin: 6px 0 0">
                      建議：{{ singleSelectedUnassignedPackage.suggested_to_node_id ?? "-" }}
                    </p>
                    <p class="hint wh-wrap" style="margin: 6px 0 0">
                      收件：{{ singleSelectedUnassignedPackage.receiver_address ?? "-" }}
                    </p>
                  </template>

                  <template v-else>
                    <p class="wh-selected-title">已選取 {{ selectedUnassignedIds.length }} 件</p>
                    <p class="hint" style="margin: 6px 0 0">
                      建議分布：
                      <span v-if="suggestedDistribution.items.length === 0">（沒有建議）</span>
                      <span v-else>
                        <span v-for="(it, idx) in suggestedDistribution.items.slice(0, 4)" :key="it.id">
                          {{ idx === 0 ? "" : "、" }}{{ it.id }} × {{ it.count }}<span v-if="!it.valid">（不在相鄰節點）</span>
                        </span>
              ...
              <div class="wh-sort-detail-scroll">
                ...
                >
                  <div v-if="activeAssignedPackages.length === 0" class="wh-empty" style="margin-top: 0">
                    <p class="wh-empty-title">此配發地目前沒有包裹</p>
                    <p class="hint wh-empty-hint">從左側拖曳包裹到此區即可分配到「{{ activeDispatchTabId }}」。</p>
                  </div>
                        ...
                        <span
                          ...
                          :title="
                            isAssignedFollowingSuggestion(p)
                              ? '已依照建議目的地分配'
                              : `未依建議分配（建議：${p.suggested_to_node_id ?? '-'}）`
                          "
                          :aria-label="
                            isAssignedFollowingSuggestion(p)
                              ? '已依照建議目的地分配'
                              : `未依建議分配，建議目的地 ${p.suggested_to_node_id ?? '-'}`
                          "
                        >
                          <svg
      ...
      <div v-show="activeTab === 'dispatched'" class="wh-panel" role="tabpanel">
        <UiCard class="wh-dispatch-workspace">
          <div class="wh-workspace-toolbar">
            <p class="hint" style="margin: 0">共 {{ dispatched.length }} 件</p>
          </div>

          <div v-if="loading" class="hint wh-gap-top-sm">載入中…</div>
          <div v-else-if="dispatched.length === 0" class="wh-empty wh-gap-top-sm" style="margin-top: 10px">
            <p class="wh-empty-title">目前沒有已派發包裹</p>
            <p class="hint wh-empty-hint">完成「派發」後，包裹會出現在此清單供後續查核。</p>
            <ul class="wh-empty-list">
              <li class="hint">切到「分揀中」完成下一跳派發</li>
              <li class="hint">點「重新整理」更新狀態</li>
            </ul>
          </div>
      ...
      <UiCard style="margin-top: 16px">
        <div class="wh-section-head">
          <div>
            ...
            <p class="hint wh-section-head__hint">僅顯示你提交的倉儲異常申報（是否已處理由客服結案）。</p>
          </div>
          <span class="hint">共 {{ exceptionReports.length }} 筆</span>
        </div>

        <div v-if="loading" class="hint wh-gap-top">載入中…</div>
        <div v-else-if="exceptionReports.length === 0" class="hint wh-gap-top">目前沒有異常申報紀錄。</div>
